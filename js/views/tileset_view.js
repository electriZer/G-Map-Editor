define(function() {
	...
	TilesetView.initialize = function() {
	    ...
	    Editor.$("#tileset_container").mouseleave(function (){Editor.$("#tileset_container .selection").remove();});
	};
	...
	TilesetView.makeSelection = function (e) {
        // block selection for Box draw and fill Draw
	    if (Editor.mousedown && e.type == "mousemove" && (Editor.tool == "drawBox" || Editor.tool == "fill")) {
		...
		if (e.type == "mouseup") {
			...
			if (Editor.tool == "drawBox" || Editor.tool == "fill") {
			    ex = sx;
			    ey = sy;
			}
			...
			{
			    Editor.$("#canvas .selection").empty(); // clear selection
			}
			...
			Editor.Canvas.drawBoxTile = Editor.$("#canvas .selection").clone();
